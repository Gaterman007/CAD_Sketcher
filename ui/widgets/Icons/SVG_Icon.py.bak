...
# Récupérer le chemin du répertoire courant (où se trouve le script)
#current_dir = os.path.dirname(bpy.data.texts["testSVG.py"].filepath)

# Ajouter ce répertoire au chemin d'importation Python
...
#    def getPixel(self,PosX, PosY):
#        return self.imageIcon.getPixel(PosX, PosY)

#    def setPixel(self,PosX, PosY, color):
#        self.imageIcon.setPixel(PosX, PosY, color)

#    def draw_line(self,x1,y1,x2,y2,color):
#        self.imageIcon.draw_line(int(x1), int(y1), int(x2), int(y2), color)
...
#        self.load_XMLFile(self.filename)
#                self.create_Image()
    ...
    def recursif_Vertices(self,element,level = 0):
        ...
        if element.type != "path" and element.type != "svg":
            ...
            if self.keepRatio:
                ...
                self.SizeY = int(self.SizeX / ratio)
            self.create_ImageIcon(self.SizeX,self.SizeY,(0.0, 0.0, 0.0, 0.0))
#            self.imageIcon = Image(width=self.finalSizeX, height=self.finalSizeY, color=(0.0, 0.0, 0.0, 0.0))
            self.deltaX = element.width / self.SizeX
            self.deltaY = element.height / self.SizeY
    ...
    def draw_paths(self, path_list):
        if self.imageIcon is not None:
            for loop in path_list:
                ...
                fixedRangeMaxX = min(int(loop.bBox[2]), self.SizeX)
                fixedRangeMinY = max(int(loop.bBox[1]), 0)
                fixedRangeMaxY = min(int(loop.bBox[3]), self.SizeY)
                # draw fill loops
                if loop.fill != 'none':
                    directionOk = True
                    if directionOk:
