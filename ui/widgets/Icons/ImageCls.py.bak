...
class Image:
    ...
    def set_image(self, rel_filename):
        ...
        rel_filename += '.png'
        rel_filepath = os.path.join("path/to/your/images", rel_filename)
    ...
    def loadFromPng(self,fileName):
        self.image = bpy.data.images.load(filepath=fileName)
        self.imageData = self.image.pixels
        self.width, self.height = self.image.size
        self.image.gl_load()
        self.texture = gpu.texture.from_image(self.image)

    def saveToPng(self, fileName, flip = False):
        if self.image is not None:
            self.image.filepath_raw = fileName
            self.image.file_format = 'PNG'
            if flip:
                self.flip_vertical()
            self.image.save_render(fileName)
