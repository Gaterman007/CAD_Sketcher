...
#        self._input_keys = ['ESC', 'RET', 'BACK_SPACE', 'HOME', 'END', 'LEFT_ARROW', 'RIGHT_ARROW', 'DEL']
    ...
    def text_input(self, event):
        ...
        if self.hasFocus:
            ...
            if event.ascii != '' and len(self.text) < self.max_input_chars:
                value = self.text[:index] + event.ascii + self.text[index:]
    ...
    def mouse_move(self, x, y):
        if self.mousePressed:
            if self.drag:
                print("dragging",self.attr)
                if self.attr is not None:
                    value = self.saveData - (float(self.downPos[0] - x) / self.increment)
                    print(value)
                    self.attributText = value
                    self.text = self.attributText
                    print(self.text)
#                bpy.context.region.tag_redraw()
                return ({"RUNNING_MODAL"},True)
            else:
                print("is_numeric",self._is_numeric)
                if self._is_numeric:
                    print("attribut is_numeric",self.attribut)
                    if abs(self.downPos[0] - x) > 5 or abs(self.downPos[1] - y) > 5:
                        print("start drag",self.attribut)
