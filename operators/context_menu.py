...
class View3D_OT_slvs_context_dialog(Operator, HighlightElement):
    ...
    bl_label = "Solvespace Context Dialog"
    ...
    def getLayoutDef(self,element,propname):
        # Parcourt les annotations (propriétés) de la classe et de ses superclasses
        for base_cls in type(element).__mro__:  # __mro__ retourne la chaîne d'héritage
    ...
    def findLayoutData(self,element,propname):
        # Parcourt les annotations (propriétés) de la classe et de ses superclasses
        for base_cls in type(element).__mro__:  # __mro__ retourne la chaîne d'héritage
            if hasattr(base_cls, '__annotations__'):
...
#                                print(f"All Keywords: {prop_info.keywords}")
    ...
    def layoutWidgets(self,element,y_pos,dialogList,panel):
        if dialogList is not None:
            for propname in dialogList:
                if isinstance(propname, list):
                    y_pos = self.layoutWidgets(element,y_pos,propname,panel)
                else:
                    for key, value in propname.items():
                        if key == "property":
                            y_pos = self.addPropToDialog(element,value,y_pos,panel)
                        elif key == "label":
                            ...
                            newwidget.setPropetyElement(element)
                            if newwidget is not None:
    ...
    def addPropToDialog(self,element,propname,ypos,panel):
        prop_info = self.getLayoutDef(element,propname)
        # create widget
        name_widget = None
        if hasattr(prop_info,"function"):
            addElement = True
            if prop_info.function.__name__ == "StringProperty":
                ...
                if prop_info.keywords['size'] > 2:
                    ...
                    panel.add_widget(newiconwidget)
                    name_widget = BL_UI_Textbox(8 + 24,ypos,280 - 24,20)
                    name_widget.setProperty(element,prop_info,2)
                    panel.add_widget(name_widget)
                    ypos += 26
                addElement = False
            else:
            ...
            if addElement:
                name_widget.setProperty(element,prop_info)
                panel.add_widget(name_widget)
                ypos += 26
        return ypos
